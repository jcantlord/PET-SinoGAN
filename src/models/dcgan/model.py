"""
Discriminator and Generator implementation based on DCGAN paper
Modified to work with 128x128x1 images
...
class Generator(nn.Module):
    def __init__(self, z_dim, channels_img, features_g):
        ...
        self.gen = nn.Sequential(
            ...
            self._block(z_dim, features_g * 16, 4, 1, 0),  # img: 4x4x1024
            self._block(features_g * 16, features_g*8, 4, 2, 1),  # img: 8x8x512
            self._block(features_g * 8, features_g*4, 4, 2, 1),  # img: 16x16x256
            self._block(features_g * 4, features_g*2, 4, 2, 1),  # img: 32x32x128
            self._block(features_g * 2, features_g, 4, 2, 1),  # img: 64x64x64
            nn.ConvTranspose2d(
                features_g, channels_img, kernel_size=4, stride=2, padding=1,
            ),
            # Output: N x channels_img x 128x128
            nn.Tanh(),
        )
